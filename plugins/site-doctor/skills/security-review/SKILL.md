...
```text
<python-command> <resolved-plugin-root>/scripts/run_helper.py security-review/scan-secrets /path/to/repo --json
```
Stdlib-only. It reports only relative path, line, rule, a short redaction, and
an irreversible SHA-256 fingerprint. It never emits a complete matching line
or secret. Manually verify each hit without asking the user to paste a value.
The helper is dispatched by the plugin-level launcher; the skill folder is not
