...
def pin_kind(spec):
    ...
    # Numeric prefixes also begin ranges such as "1.x", "1.2", and
    # "1.2.3 - 2.0.0"; only a complete semver is an exact pin.
    if EXACT_SEMVER.fullmatch(s) or (s.startswith("=") and
                                    EXACT_SEMVER.fullmatch(s[1:])):
        return "exact"
