...
def parse_tag_record(record):
    """Parse semicolon-delimited DNS policy tags using exact tag names."""
    tags = {}
    duplicates = set()
...
def _spf_record(domain):
    ...
    return next((record for record in records
                 if record.lower().startswith("v=spf1")), None)


def count_spf_dns_lookups(domain, record, visited=None):
    """Count SPF lookup mechanisms, recursively including include/redirect.

    RFC 7208's lookup budget applies to the evaluated tree, not just the top
    TXT record.  Cycles are not recursed forever; the mechanism that closes a
    cycle still counts as one lookup.
    """
    visited = set() if visited is None else set(visited)
    normalized_domain = domain.rstrip(".").lower()
    if normalized_domain in visited:
        return 0
    visited.add(normalized_domain)
    ...
    for raw_term in record.split()[1:]:
        ...
        if lower.startswith("redirect="):
            target = term.split("=", 1)[1].strip()
            if target:
                count += 1
                nested_targets.append(target)
            continue
        mechanism = re.split(r"[:/]", lower, maxsplit=1)[0]
        if mechanism in {"include", "a", "mx", "ptr", "exists"}:
            count += 1
        if mechanism == "include" and ":" in term:
            target = term.split(":", 1)[1].split("/", 1)[0].strip()
            if target:
                nested_targets.append(target)
    ...
    for target in nested_targets:
        ...
        if nested:
            count += count_spf_dns_lookups(target, nested, visited)
    return count
...
def check_spf(domain):
    ...
    # The RFC lookup budget applies recursively through include/redirect.
    lookups = count_spf_dns_lookups(domain, spf)
    if lookups > 10:
...
def check_dmarc(domain):
    ...
    dmarc = [
        ...
        if parse_tag_record(record)[0].get("v", "").lower() == "dmarc1"
    ]
    if not dmarc:
    ...
    tags, duplicates = parse_tag_record(d)
    if duplicates:
        report("FAIL", "duplicate DMARC tag(s): " + ", ".join(sorted(duplicates)))
    ...
    if policy == "reject":
        ...
        report("FAIL", "DMARC base policy p= is missing or invalid")
    if not tags.get("rua"):
