...
**Read-only reclaimable-space measurements**
```sql
PRAGMA page_count; PRAGMA freelist_count;
-- freelist_count * page_size = reclaimable bytes; report a large ratio for the explicit maintenance workflow
```

`ANALYZE`, `PRAGMA optimize`, `VACUUM`, and `REINDEX` are maintenance writes,
not audit queries. Run them only through the database-fix workflow after the
user confirms a verified backup, the exact target, and the mutation/rollback
plan.
